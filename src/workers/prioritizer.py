    ...
    with session.begin():
        ...
        if link.change_freq in [ChangeFreq.YEARLY, ChangeFreq.MONTHLY]:
            channel.basic_publish(
                exchange='',
                routing_key='low_priority_links',
                body=str(link.id)
            )
            return
            ...
            channel.basic_publish(
                exchange='',
                routing_key='medium_priority_links',
                body=str(link.id)
            )
            return
        if link.priority >= 0.7:
            channel.basic_publish(
                exchange='',
                routing_key='high_priority_links',
                body=str(link.id)
            )
            return
...
def main():
    ...
    def work(ch, method, properties, body: bytes):
        ...
        try:
            ...
            logger.critical(f"[{type(e).__name__}] - Lost {link_id} inside a prioritizer worker due to : {e}")

    channel.basic_consume(queue='prioritizer', on_message_callback=work)

    try:
        print(' [prioritizer] [{worker_id}] Waiting for links to prioritize. To exit press CTRL+C')
